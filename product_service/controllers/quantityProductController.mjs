...
export const getQuantityByProductId = async (req, res) => {
  try {
    const { productId } = req.params;
    const quantities = await Quantity_Product.find({ product_id: productId });

    if (!quantities || quantities.length === 0) {
      return res
        .status(404)
        .json({ message: "No quantities found for this product" });
    }
...
export const getQuantityByProductIdAndSize = async (req, res) => {
  try {
    ...
    let quantity;
    if (size) {
      quantity = await Quantity_Product.findOne({
        product_id: productId,
        size: size,
      });
    } else {
    ...
    if (!quantity) {
      return res
        .status(404)
        .json({ message: "Quantity not found for this product and size" });
    }
